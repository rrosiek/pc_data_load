...
from data_load.uspto_2001.uspto_data_mapper import USPTODataMapper
from data_load.uspto_2001.uspto_data_extractor import USPTODataExtractor
...
class USPTOLoadManager(LoadManager):
    ...
    def mapping_file_path(self):
        return 'data_load/uspto_2001/mapping.json'
    ...
    def download_data(self):
        ...
        if self.year is not None:
            self.files_to_process = file_manager.download_files(load_config, year=self.year, pre_2001=True)
        else:
            self.files_to_process = file_manager.get_files_to_process(load_config)

            # print 'Files to process', json.dumps(self.files_to_process)
            if len(self.files_to_process) == 0:
                year = file_manager.get_next_year(load_config, pre_2001=True)
                self.files_to_process = file_manager.download_files(load_config, year=year, pre_2001=True)
