...
class PubmedDataMapper(DataMapper):
    ...
    def get_citations(data):
        ...
        citations = []
        if 'MedlineCitation' in doc:
            if 'CommentsCorrectionsList' in doc['MedlineCitation']:
                if 'CommentsCorrections' in doc['MedlineCitation']['CommentsCorrectionsList']:
                    cc = doc['MedlineCitation']['CommentsCorrectionsList']['CommentsCorrections']

                    if isinstance(cc, list):
                        for cc_item in cc:
                            if 'RefType' in cc_item:
                                ref_type = cc_item['RefType']
                                if ref_type == 'Cites':
                                    if 'PMID' in cc_item:
                                        pmid_obj = cc_item['PMID']
                                        if isinstance(pmid_obj, dict):
                                            if 'content' in pmid_obj:
                                                pmid = pmid_obj['content']
                                                citations.append(pmid)
                                        else:
                                            citations.append(pmid_obj)
                    else:
                        cc_item = cc
                        if 'RefType' in cc_item:
                            ref_type = cc_item['RefType']
