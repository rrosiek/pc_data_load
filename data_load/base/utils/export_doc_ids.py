...
def get_doc_ids_for_load_config(load_config, query=None):
    dest_dir = load_config.other_files_directory()
    dest_file_name = 'DOC_IDS_' + load_config.index + '.json'
    documents_ids = get_doc_ids(load_config.server, load_config.index, load_config.type, dest_dir, dest_file_name, query)
    return documents_ids
