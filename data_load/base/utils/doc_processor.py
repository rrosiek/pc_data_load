from data_load.base.utils import data_utils
...
class DocProcessor(object):
    ...
    def run(self):
        ...
        data_utils.batch_fetch_ids_for_query(base_url=self.server, index=self.src_index, type=self.src_type, query=query, ids_fetched=self.ids_fetched)
    ...
    def ids_fetched(self, ids, index, type):
        data_utils.batch_fetch_docs_for_ids(base_url=self.server, ids=ids, index=self.src_index, type=self.src_type, docs_fetched=self.docs_fetched)
